...
export const addFeedsToStock = async (req, res, next) => {
    ...
    try {
        ...
        const fetchCurrentRecordQuery = `SELECT amount_left FROM feeds_inv WHERE id = ?`;
        ...
        // Step 4: Fetch the updated record to get item_type and item_name
        ...
        // Step 5: Fetch the last record for the same item_type and item_name
        ...
        if (lastRecord) {
            ...
            // Step 6: Calculate the new amount_left for the new record
...
export const addSupplementsToStock = async (req, res, next) => {
    ...
    try {
        ...
        // Step 1: Fetch the current record to check the amount_left
        const fetchCurrentRecordQuery = `SELECT amount_left FROM supplements_inv WHERE id = ?`;
        const [currentRecordRows] = await connection.query(fetchCurrentRecordQuery, [id]);
        const currentRecord = currentRecordRows[0];
